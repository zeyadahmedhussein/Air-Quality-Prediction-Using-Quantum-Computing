...
# Import the model and device management from modules
import sys
import os
...
def load_model_and_config(model_path, config_path=None):
    """Load trained quantum model and its configuration with new device management"""
    # Load model config
...
def load_and_preprocess_data(dataset_path, preprocessing_state_path):
    ...
    from preproccessing import preprocess_for_inference
...
def run_quantum_inference(model, data, backend_name=None, shots=2048, 
                         optimization_level=3, seed_transpiler=42, device_type=None):
    """Run quantum inference on the model using the configured device manager"""
    ...
    # If overrides provided, update model's quantum device accordingly
    if backend_name or device_type or shots:
        dm = QuantumDeviceManager(
            backend_name=backend_name or getattr(model.device_manager, 'backend_name', None),
            shots=shots,
            use_noise_model=(device_type == 'noisy') if device_type is not None else getattr(model.device_manager, 'use_noise_model', False),
            use_real_hardware=(device_type == 'hardware') if device_type is not None else getattr(model.device_manager, 'use_real_hardware', False)
        )
        model.device_manager = dm
        model.quantum_device = dm.create_device(model.n_qubits, device_type or model.device_type)
...
def main():
    ...
    predictions, job_id, inference_time = run_quantum_inference(
        ...
        args.optimization_level, args.seed_transpiler, device_type=None
    )
